...
const OverallAttendance = () => {
  ...
  const updateRecord = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/attendance/api/overall_attendance_record/${editRecord.id}`,
        editRecord
      );
  ...
  const deleteRecord = async (id) => {
    await axios.delete(
      `http://localhost:5000/attendance/api/overall_attendance_record/${id}`
    );
    fetchAttendanceData();
    ...
    try {
      ...
      for (const record of attendanceData) {
        ...
        if (response.data.exists) {
          ...
          alert(
            `Existing payroll entry found for Employee Number ${personID} from ${startDate} to ${endDate}. Submission cancelled.`
          );
          return;
        }
      }
