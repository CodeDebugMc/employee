...
const AttendanceModuleFaculty = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const processedData = response.data.map((row) => {
        ...
        const minutesFaculty = Math.floor(
        ...
        const formattedFacultyRenderedTime = [
          String(hoursFaculty).padStart(2, '0'),
          String(minutesFaculty).padStart(2, '0'),
          String(secondsFaculty).padStart(2, '0'),
        ].join(':');
        ...
        const minutesFacultyMRT = Math.floor(
          (diffMsFaculty % (1000 * 60 * 60)) / (1000 * 60)
        );
        const secondsFacultyMRT = Math.floor(
          (diffMsFaculty % (1000 * 60)) / 1000
        );

        // Format output as HH:MM:SS
        const formattedFacultyMaxRenderedTime = [
          String(hoursFacultyMRT).padStart(2, '0'),
          String(minutesFacultyMRT).padStart(2, '0'),
          String(secondsFacultyMRT).padStart(2, '0'),
        ].join(':');

        // Calculate difference in milliseconds MAX RENDERED TIME

        const tardFinalformattedFacultyRenderedTime = new Date(
          `01/01/2000 ${formattedFacultyRenderedTime}`
        );
        const tardFinalformattedFacultyMaxRenderedTime = new Date(
          `01/01/2000 ${formattedFacultyMaxRenderedTime}`
        );

        const finalcalcFaculty =
          tardFinalformattedFacultyMaxRenderedTime -
          tardFinalformattedFacultyRenderedTime;

        // Convert milliseconds to hours, minutes, seconds
        const hoursfinalcalcFaculty = Math.floor(
          finalcalcFaculty / (1000 * 60 * 60)
        );
        const minutesfinalcalcFaculty = Math.floor(
          (finalcalcFaculty % (1000 * 60 * 60)) / (1000 * 60)
        );
        const secondsfinalcalcFaculty = Math.floor(
          (finalcalcFaculty % (1000 * 60)) / 1000
        );

        // Format output as HH:MM:SS
        const formattedfinalcalcFaculty = [
          String(hoursfinalcalcFaculty).padStart(2, '0'),
          String(minutesfinalcalcFaculty).padStart(2, '0'),
          String(secondsfinalcalcFaculty).padStart(2, '0'),
        ].join(':');
  ...
  const calculateTotalRenderedTime = () => {
    ...
    attendanceData.forEach((row) => {
      ...
      const [hours, minutes, seconds] = facultyRenderedTime
        .split(':')
        .map(Number);
    ...
    const overallTime = `${String(totalHours).padStart(2, '0')}:${String(
      totalMinutes
  ...
  const calculateTotalRenderedTimeTardiness = () => {
    ...
    attendanceData.forEach((row) => {
      ...
      const [hours, minutes, seconds] = facultyRenderedTimeTardiness
        .split(':')
        .map(Number);
      totalSeconds += hours * 3600 + minutes * 60 + seconds;
    });
              ...
              <TableBody>
                ...
                <TableRow>
                  <TableCell
                    ...
                    sx={{ fontWeight: 'bold', textAlign: 'right' }}
                  >
                    Overall Rendered Official Time <br /> {startDate} to{' '}
                    {endDate}:
                  </TableCell>
                  <TableCell
                    colSpan={2}
                  ...
                  <TableCell
                    ...
                    sx={{ fontWeight: 'bold', textAlign: 'right' }}
                  >
                    Overall Tardiness Ofiicial Time <br /> {startDate} to{' '}
                    {endDate}:
                  </TableCell>
                  <TableCell
                    colSpan={2}
