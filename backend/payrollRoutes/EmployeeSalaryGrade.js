...
router.post('/employee-salary-grade', (req, res) => {
  ...
  const sql =
    'INSERT INTO employee_salary_grade (employeeNumber, plantilla_id, step, created_at) VALUES (?, ?, ?, ?)';
  db.query(
    sql,
    [employeeNumber, plantilla_id, step, created_at],
    (err, result) => {
...
router.put('/employee-salary-grade/:id', (req, res) => {
  ...
  const { id } = req.params;
  const sql =
    'UPDATE employee_salary_grade SET employeeNumber = ?, plantilla_id = ?, step = ?, created_at = ? WHERE id = ?';
  db.query(
    sql,
    [employeeNumber, plantilla_id, step, created_at, id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
