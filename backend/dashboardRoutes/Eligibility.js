...
router.post('/upload_eligibility', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const insertPromises = sheet.map((row) => {
      ...
      const eligibilityDateOfExam = excelDateToUTCDate(
        row.eligibilityDateOfExam
      );
      const formattedDateOfExam = eligibilityDateOfExam
        .toISOString()
        .split('T')[0];
      ...
      const formattedDateOfValidity =
        DateOfValidity.toISOString().split('T')[0];
      const person_id = row.person_id;

      const sql =
        'INSERT INTO eligibility_table (eligibilityName, eligibilityRating, eligibilityDateOfExam, eligibilityPlaceOfExam, licenseNumber, DateOfValidity, person_id) VALUES (?, ?, ?, ?, ?, ?, ?)';
...
router.post('/eligibility', (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      res
        .status(201)
        .send({ message: 'Eligibility created', id: result.insertId });
    }
  );
});
...
router.put('/eligibility/:id', (req, res) => {
  ...
  const { id } = req.params;
  const query =
    'UPDATE eligibility_table SET eligibilityName = ?, eligibilityRating = ?, eligibilityDateOfExam = ?, eligibilityPlaceOfExam = ?, licenseNumber = ?, DateOfValidity = ?, person_id = ? WHERE id = ?';
  db.query(
    query,
    [
      eligibilityName,
