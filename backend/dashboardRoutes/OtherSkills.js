...
router.post('/other-information', (req, res) => {
  ...
  const query =
    'INSERT INTO other_information_table (specialSkills, nonAcademicDistinctions, membershipInAssociation, person_id) VALUES (?, ?, ?, ?)';
  db.query(
    query,
    [
      specialSkills,
...
router.put('/other-information/:id', (req, res) => {
  ...
  const query =
    'UPDATE other_information_table SET specialSkills = ?, nonAcademicDistinctions = ?, membershipInAssociation = ?, person_id = ? WHERE id = ?';
